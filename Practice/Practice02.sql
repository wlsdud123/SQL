...
select  department_id,
        ...
        max(salary), -- department_id(최고임금)
        min(salary), -- department_id(최저임금)
...
select  job_id,
        round(avg(salary),0) "r0", -- job_id(평균)
        max(salary), -- job_id(최고임금)
        min(salary) -- job_id(최저임금)
...
select  department_id,
        round(avg(salary),0) "r0", -- job_id(평균임금)
        min(salary), -- job_id(최저임금)
